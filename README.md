...
- **知识问答** — 自然语言健身问题，162 篇知识文档混合检索，RRF 融合 + Re-rank 精排，带来源引用
  ...
  安全关键内容不走生成路径，且安全判定不依赖知识库——FactChecker 的规则引擎是独立的伤病冲突表（47 关键词 → 200 禁止对）+ 15 份语义档案，删掉整个知识库它仍能拦截高危组合。
  块数 **557 chunks** 取自 2026-08-30 摄入输出、与评测 manifest 同源，但**静态清单无法核验**（数据在 PostgreSQL，需重跑摄入才能复测）。
