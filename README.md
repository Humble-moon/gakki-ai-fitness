...
计划生成时安全审查是**分流**的，不是一律放行也不是一律拦截：

| 命中 | 处置 |
|---|---|
| 伤病-动作冲突（规则引擎，确定性） | **扣下送审**，交付审核工件而非计划 |
| embedding 语义匹配到伤病档案 | 扣下送审 |
| LLM 审查报 danger，或置信度 < 0.7 | 扣下送审 |
| **急症信号**（胸闷/头晕/晕厥/心悸/呼吸困难） | 走**独立于伤病**的急症话术，**禁止任何训练建议**，要求立即停止并就医 |
| 仅 warning 级建议（"新手注意恢复"这类） | **照常交付**，同时作为警告展示给用户 |
| 计划含动作库中不存在的动作名 | **照常交付**，只提示「请确认动作名称」 |

最后两行是实测教训：LLM 检查器几乎从不返回空 issue 列表，若把 warning 也当作送审判据，**所有计划都会进审核队列**；而动作库外动作里混着大量命名变体（`保加利亚分腿蹲（扶支撑）`）与真实但库未收录的动作，一律拦截等于让正常计划交付不出去。两者都会让用户拿不到任何计划。

**急症词表单独成表**的原因：伤病词表全是"疼/痛/伤"一类亚急性描述，而"胸闷"连"痛"字都没有——实测输入「训练时突然胸闷、头晕，眼前发黑」时，安全话术根本不会注入。两者的处置也不同：伤病允许给运动康复建议，急症必须禁止任何训练指导，因此**不能叠加注入**。
...
| 安全 | 三层安全门（伤病规则引擎 + embedding 语义匹配 + LLM 审查）+ **急症词表**（独立于伤病词表，命中即禁止给训练建议）+ **动作库校验**（计划中的动作名必须在库）+ HITL |
| 成本与数据 | 当日 LLM 成本上限（Redis 计数，跨进程有效）；个人数据导出与删除 |
| 记忆 | 短期滑动窗口 + 长期记忆（带时间戳） |
