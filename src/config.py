...
# 出网文本脱敏（直接标识符剥离）
#
# 用户伤病描述会随 prompt 发往外部 LLM API、随查询发往 embedding API。
# 真正的隐私风险不是「腰椎间盘」这个诊断，而是能把记录关联到具体自然人的
# 直接标识符：姓名、手机号、精确就诊日期、病历号、证件号。
#
# 采用「最小必要披露」而不是全遮蔽：把伤病整体替换成 INJURY_TYPE_BACK_003
# 这类不透明 ID，会让 FactChecker 的 LLM 语义审查失去判断依据、让 HITL 的
# 语义匹配（15 个伤病档案、余弦阈值 0.68）整体失效——等于用隐私收益换掉
# 安全能力。因此只剥离标识符，完整保留医学语义。
#
# 脱敏点放在 LLMProvider 出口（文本离开本机的最后一跳）：hitl/review.py 的
# 规则引擎与语义匹配都是本地逻辑、不出网，需要看到完整原文。
# ============================================================
LLM_INJURY_REDACTION = os.getenv("LLM_INJURY_REDACTION", "off").lower()
# off（默认，出网文本与历史行为一致）| on
# 开启会改变送给 LLM 的 prompt，属于影响生成质量与评测指标的变更，
# 启用后应重跑相关评测再与论文基线对比。
