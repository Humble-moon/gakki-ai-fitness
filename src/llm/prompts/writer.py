...
【强制约束】你必须严格按照用户画像中指定的每周训练次数生成对应数量的训练日。
用户说几天就几天，不要自作主张增减。如果用户要求 5 天，days 数组里必须恰好有 5 个元素。
计划总周数必须来自用户画像或计划配置提供的确定值；如果上下文没有提供，weeks 输出 null，不得猜测或伪造。

输出 JSON 格式（严格遵守键名）：
{
  "plan_name": "计划名称",
  "goal": "增肌或减脂",
  "weeks": <按上下文提供的计划总周数；未提供则为 null>,
  "sessions_per_week": <按用户画像中的每周训练次数>,
  "days": [
    {
      "day": 1,
      "focus": "训练重点",
